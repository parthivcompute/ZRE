...
Temporal drift detection (distribution changes over time)
Lightweight LSTM for short term signal estimation
...
Temporal drift (changes in clustering distribution over time)
...
This produces a stable, normalized score that responds to both sudden anomalies and gradual behavioral shifts.
...
Transformer vs household load comparison
...
CSV logging of zone activity
FastAPI backend for real time ingestion (simulated)
...
The system operates on aggregated energy measurements, which introduces a key limitation.

After Meter Theft Problem
Grid flow: Transformer → Meter → House
...
This is not a model failure, but a limitation of measurement granularity.

System Evolution: Line 
Level Sensing
Proposed extension:
...
Reduced ambiguity in anomaly detection
Transition from detection to precise fault identification
How to Run
Backend: python zre_backend.py
...
Visualization
Alert generation
It uses simulated input streams to validate anomaly detection logic and system behavior before real world deployment.
...
from detecting anomalies → to understanding where and why they occur.